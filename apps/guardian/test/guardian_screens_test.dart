...
void main() {
  ...
  setUp(() async {
    ...
    dependencies = await AppDependencies.create(
      ...
      api: api,
      // ✅ م.7.4: قارئُ التوكن يُزيَّف صراحةً ولا يُترك للافتراضي.
      //
      // الافتراضيُّ `readPushToken` يلمس Firebase الحقيقية، ولا يُستدعى إلا في
      // `signIn` — فهذه الاختباراتُ تمرّ بدونه اليوم **مصادفةً**. واختبارٌ يُضاف
      // غداً ويستدعي الدخولَ كان سيصطدم بقناةِ منصّةٍ لا تعمل في اختبار وحدة،
      // فيفشل لسببٍ لا علاقة له بما يقيسه.
