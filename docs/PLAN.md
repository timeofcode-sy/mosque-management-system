...
> **حالة الخطة:** محدّثة بعد تنفيذ المراحل 0 إلى 5.4 — **المرحلة 5 مكتملة، وأوّلُ تطبيق فلاتر
> يعمل على جهاز**. ما نُفِّذ موسوم بـ ✅، وما تغيّر عن الخطة الأصلية موسوم بـ 🔄 مع سبب التغيير.
...
> عميقة ولا مسارات مسمّاة. فبقي على `Navigator` و`InheritedWidget` واحد. تُعاد المسألة عند
> الديسكتوب (م.6) إن احتاج تنقّلاً أعقد — لا قبله.

**`packages/mousqe_core`** ✅ (يستخدمه الأربعة): نماذج freezed مطابقة لمخطط الخادم، مخطط drift
المحلي، `SyncEngine` (طابور عمليات مرتَّب + إعادة محاولة + تمييز الانقطاع عن الخطأ)، `ApiClient`،
تخزين آمن للتوكن. و**`packages/mousqe_ui`** ✅: ثيم المعهد الثلاثي وRTL والمكوّنات المشتركة.
...
| **🔴 المشرف بلا طريق إلى الـ API** | 2026-09-06: `InviteUser` ينشئ حساب `supervisor` بلا سجلّ `teacher`/`guardian`/`student` (عمداً)، و`ApiScope` يرفض من لا سجلّ له بـ422 ⇒ **تطبيق الديسكتوب (م.6) لا يستطيع الدخول** إلا لمشرفٍ هو أستاذ مسجَّل أيضاً. **حاجبٌ للمرحلة 6 — أوّلُ ما يُحسم فيها** — [CLIENTS.md §5](CLIENTS.md) |
...
| **⚠️ عمليةٌ مسمومة توقف طابور المزامنة كلَّه** | 2026-09-06 (م.5.3): الدفعة تُرسَل كتلةً واحدة، وفشلُ واحدةٍ بـ500 يُنهي الطلب. التطبيق يعرض الخطأ ولا يحذف شيئاً (وهو الصواب) لكنّ المزامنة تتوقّف حتى تدخّلٍ بشري. احترامُ `editable` محلياً يمنع السببَ الأشيع؛ عزلُ العملية المسمومة بندٌ في م.6 — [SYNC-PROTOCOL.md §10](SYNC-PROTOCOL.md) البند 10 |
