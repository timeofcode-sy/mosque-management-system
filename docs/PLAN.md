...
> **حالة الخطة:** محدّثة بعد تنفيذ المراحل 0 إلى **7.1** — **المرحلتان 5 و6 مكتملتان**: تطبيقُ
> الأستاذ يعمل على جهاز، وتطبيقُ الديسكتوب **خمسةَ عشرَ باباً من خمسةَ عشر** وبناؤه الأصلي على
> ويندوز مُجرَّب. **والمرحلة السابعة (تطبيق ولي الأمر) قيد التنفيذ**: أساسُها الخادمي منفَّذ
> (7.1)، وتبقى الحزمةُ (7.2) والتطبيقُ (7.3) والإشعارُ الفوري (7.4 — معلَّقةٌ على حساب Firebase).
...
> و[CHECKPOINT-PHASE-7.1.MD](CHECKPOINT-PHASE-7.1.MD).
>
...
| 7.1 | **الأساس الخادمي لولي الأمر** | `session_date` في سجلّ الحضور · نقطتان جديدتان (الأعذارُ وتقدُّمُ الحفظ) · `ProgressResource` و`AbsenceExcuseResource` · `null` لا صفر في المنحنى · نزعُ `reports.view` | يوم | ✅ **منفَّذة** — [CHECKPOINT-PHASE-7.1.MD](CHECKPOINT-PHASE-7.1.MD) |
| 7.2 | **`mousqe_core`: مستودعُ ولي الأمر** | خمسُ نقاطٍ في `ApiClient` · `GuardianRepository` يقرأ REST ويخزّن اللقطة · بلا `SyncEngine` | يوم | ⬜ |
| 7.3 | **تطبيق الأهل** | `apps/guardian/` — خمسُ شاشات فوق الحزمتين | ٢–٣ أيام | ⬜ |
| 7.4 | **الإشعارُ الفوري** | مُرسِلٌ خادميٌّ خلف `NotificationDriver` + استقبالٌ في التطبيق | يوم | ⏸ **معلَّقة على حساب Firebase** |
...
| **حساب Firebase** | ⏸ 🔄 **م.7: لم يعد حاجباً للمرحلة، بل لمرحلتها الفرعية الأخيرة وحدها (7.4).** تُبنى 7.1→7.3 كاملةً بدونه ويُسلَّم التطبيقُ عاملاً — يفتحه وليُّ الأمر فيرى الغيابَ لا أن يُبلَّغ به. نقصٌ في الفورية لا في الوظيفة — [PHASE-7-STAGES.MD §0.2](PHASE-7-STAGES.MD) |
