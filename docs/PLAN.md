...
> **حالة الخطة:** محدّثة بعد تنفيذ المراحل 0 إلى 5.1.
...
> [CHECKPOINT-PHASE-4.7.MD](CHECKPOINT-PHASE-4.7.MD)، و[CHECKPOINT-PHASE-5.1.MD](CHECKPOINT-PHASE-5.1.MD).
>
...
| 5.2 | **الحزمتان المشتركتان** | `mousqe_core` (freezed · drift · ApiClient · SyncEngine) و`mousqe_ui` (ثيم المعهد · RTL · مكوّنات) | ٣–٤ أيام | ⬜ التالية — [PHASE-5-STAGES.MD §3](PHASE-5-STAGES.MD) |
| 5.3 | **تطبيق الأستاذ** | ثماني شاشات أوف-لاين كاملة فوق الحزمتين | ٣–٤ أيام | ⬜ [PHASE-5-STAGES.MD §4](PHASE-5-STAGES.MD) |
...
php artisan test --compact                # 279/279 حالياً
...
| **🔴 كتابات اللوحة لا تدخل `change_log`** | 2026-09-06: `RecordChange` مستدعىً من `SyncPush` وحده — فما يُكتب من اللوحة (تسجيل طالب، تعديل تفقّد، مراجعة إذن) **لا يصل أي عميل**، خلافاً لما يصفه §5. **حاجب للمرحلة 5** — [SYNC-PROTOCOL.md §10](SYNC-PROTOCOL.md) |
| **🔴 المشرف بلا طريق إلى الـ API** | 2026-09-06: `InviteUser` ينشئ حساب `supervisor` بلا سجلّ `teacher`/`guardian`/`student` (عمداً)، و`ApiScope` يرفض من لا سجلّ له بـ422 ⇒ **تطبيق الديسكتوب (م.6) لا يستطيع الدخول** إلا لمشرفٍ هو أستاذ مسجَّل أيضاً — [CLIENTS.md §5](CLIENTS.md) |
| **⚠️ `user.roles` فارغة في `/auth/login` و`/auth/me`** | 2026-09-06: المساران خارج `institute.scope` فمفتاح فريق spatie غير مضبوط ⇒ لا دور يعود. لا تبنِ توجيه التطبيقات عليه — [API.md §8](API.md) |
...
| **⚠️ `conflicts.review` صلاحية لا تحرس شيئاً** | 2026-09-06: شاشة التعارضات خلف `system.debug` (المبرمج وحده) بينما الصلاحية مبذورة للمشرف — يُحسم أيّهما الصواب قبل م.6 |
| **⚠️ ثلاثة فروع دفع بلا تحقّق من المعهد** | 2026-09-06: `attendance.session.complete` و`attendance.teacher.take` و`recitation.delete` تبحث بـ`uuid` وحده. الخطر منخفض (uuid v7 غير قابل للتخمين) والإصلاح سطر لكل فرع |
