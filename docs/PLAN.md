...
> **حالة الخطة:** محدّثة بعد تنفيذ المراحل 0 إلى **8.2 — والتطبيقاتُ الأربعة مكتملة** — **المرحلتان 5 و6 مكتملتان**: تطبيقُ
...
| 8.2 | **تطبيق الطالب** | `apps/student/` — خمسُ شاشات · **ورفعُ المشترك إلى `mousqe_ui` بدل نسخه** | يومان | ✅ **منفَّذة** — 231 في Dart — [CHECKPOINT-PHASE-8.2.MD](CHECKPOINT-PHASE-8.2.MD) |
| — | **✅ المرحلة 8 مكتملة** | **وبها تكتمل التطبيقاتُ الأربعة**: الأستاذ · الديسكتوب · وليُّ الأمر · الطالب | — | ✅ [PHASE-8-STAGES.MD](PHASE-8-STAGES.MD) |
...
| 2026-09-10 | 🔑 **ما بدا تعارضاً بين قاعدتين كان سؤالاً لم يُطرح** (م.8.2). سجّلت م.7.3 أن ترجمةَ `AttendanceStatus` إلى `BadgeStatus` **لا تُرفَع** رغم تكرارها ثلاثَ مرّات، لأن `mousqe_ui` لا تعتمد `mousqe_core` ورفعَها إلى الأخيرة يقلب الاتجاه إلى ما هو أسوأ — فكان التكرارُ «ثمنَ حدٍّ مقصود». وكانت هذه المرحلةُ ستجعلها رابعة. **والمخرجُ أن القيمةَ النصّية عقدٌ قائمٌ منذ م.1**: `'present'` تعبر الشبكةَ وتسكن قاعدةَ الخادم، فتقرؤها `mousqe_ui` بـ`BadgeStatus.fromValue` ولا تحتاج أن تعرف الـenum. **والدرس: قاعدتان تتعارضان ظاهرياً تعنيان غالباً أن السؤال الصحيح لم يُطرح بعد — أيُّ عقدٍ يجمع الطرفين بلا أن يعرف أحدُهما الآخر؟** |
| 2026-09-10 | 🔁 **سجلّاتُ Dart تعبر حدَّ الحزم بلا أن تنقضه** — `SnapshotView` يحتاج `Snapshot<T>` وهو في `mousqe_core`، و`mousqe_ui` لا تعتمدها. فأعلنت حزمةُ العرض الشكلَ (`typedef Loaded<T> = ({T value, DateTime? fetchedAt, bool isStale})`) وأعطته حزمةُ البيانات (`Snapshot.ui`)، **فيتطابقان بلا استيراد** لأن السجلّات تُطابَق بشكلها لا باسمها. وبه رُفعت خمسةُ مكوّناتٍ من `apps/guardian/` إلى `mousqe_ui` بدل نسخها إلى تطبيق الطالب — **واختباراتُ ولي الأمر الثمانية مرّت بلا تعديلِ توكيدٍ واحد** |
| 2026-09-10 | ✅ **المرحلة 8.2 — تطبيق الطالب، وبها تكتمل التطبيقاتُ الأربعة** ([CHECKPOINT-PHASE-8.2.MD](CHECKPOINT-PHASE-8.2.MD)). `apps/student/` بخمس شاشات: الرئيسيةُ برتبته ونسبته ونقاطه، وحضوري وحفظي والإعلانات. **والرتبةُ تُعرض بثلاث حالاتٍ صريحة** — مقيسةٌ «5 من 20»، وغيرُ مقيسةٍ تقول ذلك ولا تُذيَّل، وبلا حلقةٍ جملةٌ مفهومة. **231** اختباراً في Dart (كانت 222) في **ستّ** حزم، و437 خادمياً بلا تغيير، وAPK يخرج بشعار المعهد |
