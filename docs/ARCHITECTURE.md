...
الحالة: **2026-09-10 · محدَّث حتى المرحلة 8.2** — الباك إند واللوحة منفَّذان بالكامل
(المراحل 0–5.4)، والحزمتان المشتركتان وتطبيقُ الأستاذ ✅ منفَّذة، و**تطبيقُ الديسكتوب ✅ منفَّذ
كاملاً** (م.6.1–6.6): خمسةَ عشرَ باباً من خمسةَ عشر، وبناؤه الأصلي على ويندوز مُجرَّب.
**وتطبيقُ ولي الأمر ✅ منفَّذ** (م.7.1–7.3) بخمس شاشات ومعه إشعارُه الفوري (م.7.4)،
✅ **وتطبيقُ الطالب** (م.8.1–8.2) بخمس شاشات — **فالتطبيقاتُ الأربعة مكتملة**.
...
└─ apps/
     ...
     ├─ admin_desktop/ ✅ م.6.1–6.6 · Windows ├─ REST /api/v1 + مزامنة أوف-لاين
     ├─ guardian/    ✅ م.7.1–7.3 · Android   │  (بلا مزامنة — REST + لقطات)
     └─ student/     ✅ م.8.1–8.2 · Android    ┘  (بلا مزامنة — REST + لقطات)
```
...
| **الديسكتوب** | ✅ م.6.1–6.6 | REST `/api/v1` + `sync/push` + `sync/pull` | ✓ كامل | drift/SQLite — جداولُ الدومين |
| **الأهل** | ✅ م.7.1–7.3 | 🔄 **`/guardian/*` وحدها — بلا `sync/pull` وبلا `push`** | ✓ قراءة | drift/SQLite — **مخزنُ لقطات** |
| **الطالب** | ✅ م.8.1–8.2 | 🔄 **`/student/me/*` وحدها — بلا `sync/pull` وبلا `push`** | ✓ قراءة | drift/SQLite — **مخزنُ لقطات** |
...
🔄 **م.7.1: وليُّ الأمر خرج من `sync/pull` أيضاً** — وهو أوّلُ عميلٍ يفعل. التيّارُ يبثّ **معهداً
كاملاً** لكل الأدوار ([SYNC-PROTOCOL.md §4](SYNC-PROTOCOL.md))، وذاك على جهاز الطاقم حِملُ أداءٍ
مؤجَّلُ القياس، وعلى جهاز الأب **تسريب**: أسماءُ كلِّ طلاب المعهد وحضورُهم في هاتف كلِّ وليّ.
ولا يحتاجه أصلاً — من يستدعي التيّارَ من **يكتب** في الطابور.

**وأثرُه على المخزن**: ما يصل عبر نقطةٍ مخصّصة لا يسكن جداولَ الدومين بل **مخزنَ لقطات**
(`app_state`)، لأنه لا يُستعلَم عليه بل يُعرض كما وصل — وهي صياغةٌ ثانية لقاعدة م.6.5 «قناةُ
الكتابة تحدّد المستودع»، وهنا تحدّده **قناةُ القراءة**
([CHECKPOINT-PHASE-7.2.MD §2](CHECKPOINT-PHASE-7.2.MD)).

✅ **م.8.1: والطالبُ خرج منه كذلك** — وحجّتُه أقوى (يقرأ عن نفسه وحدها). وما كان يعوزه أن
**ترتيبَه في الحلقة يُحسب محلياً من صفوف زملائه**، فقُلب الاتجاه: الخادمُ يحسب الرتبةَ ويعيد
«أنت الخامس من عشرين» — **ولا صفَّ عن زميلٍ واحد**
([CHECKPOINT-PHASE-8.1.MD §2](CHECKPOINT-PHASE-8.1.MD)).

فبقيت الفجوةُ السابعة **للأستاذ والديسكتوب وحدهما** — وهما طاقمٌ فهي عندهما حِملٌ لا تسريب.
