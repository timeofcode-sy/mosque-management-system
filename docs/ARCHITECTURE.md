...
الحالة: **2026-09-09 · محدَّث حتى المرحلة 6.6** — الباك إند واللوحة منفَّذان بالكامل
(المراحل 0–5.4)، والحزمتان المشتركتان وتطبيقُ الأستاذ ✅ منفَّذة، و**تطبيقُ الديسكتوب ✅ منفَّذ
كاملاً** (م.6.1–6.6): خمسةَ عشرَ باباً من خمسةَ عشر، وبناؤه الأصلي على ويندوز مُجرَّب. ويبقى
⬜ تطبيقا م.7 (ولي الأمر) وم.8 (الطالب).
...
| `attendance.session.lock` ✅ م.6.4 | `LockAttendanceSession` | ✅ `pages::attendance.take` — وكان **يستدعيها وحدها** حتى فُتح له نوعُه |
...
✅ **م.6.5: وبقي البابُ واحداً حين اكتمل السطح.** `SaveCurriculum` **أُخرج** من شاشة المناهج التي
كانت تكتب `Curriculum::updateOrCreate` مباشرةً، فصار السطحان يستدعيانه — وهو خامسُ فعلٍ يُخرَج بنفس
القاعدة. **وكسبُه لم يكن في الأسطر بل في عطبٍ ظهر لحظةَ إخراجه:** الشاشةُ كانت تُسند
`institute_id` بلا شرط، فتحريرُ منهجٍ **عامّ** من معهدٍ واحد كان ينقله إلى ملكِه فيختفي — بأجزائه
وسجلّاتِ التقدّم المعلّقة عليها — عن كلِّ معهدٍ آخر. والحكمُ وُضع في الفعل لا في المتحكّم، فسرى على
السطحين معاً في نفس الالتزام ([CHECKPOINT-PHASE-6.5.MD §3](CHECKPOINT-PHASE-6.5.MD)).
...
| الـ API (`api`) | `auth:sanctum` → `EnsureUserIsActive` → `institute.scope` → `permission:`/`role:` | ✅ م.6.1: كلُّ حسابٍ له **سجلّ** `teacher`/`guardian`/`student` **أو دورٌ مسنَد داخل معهد** — أي جمهور التطبيقات الأربعة معاً |

✅ **م.6.1 — نُقضت عمداً النتيجةُ التي كانت مسجَّلةً هنا.** كانت: «`admin` و`super_admin`
و`developer` لا يعبرون الـ API إطلاقاً» — لا منعاً بصلاحية، بل لأن `ApiScope` يحسم المعهد من
`user->teacher ?? user->guardian ?? user->student` وحدها، وحسابُ مدير المعهد لا يحمل أياً منها
فيُرفض بـ422؛ فكان `supervisor` لا يصله إلا إن كان **أيضاً** أستاذاً مسجَّلاً.

نقضُها كان قرارَ صاحب المشروع (2026-09-07): الديسكتوب يخدم **المشرف ومديرَ المعهد والمشرفَ الأعلى
والمبرمج معاً، بما تحتويه اللوحة كاملاً** ([APPS-FEATURES.md §4](APPS-FEATURES.md)). ونُفِّذ في
م.6.1 بأن صار **إسنادُ الدور داخل معهد** طريقاً ثانياً إلى النطاق، ومعه مبدّلُ معاهدَ بترويسة
`X-Institute` لحاملي الأدوار العابرة ([API.md §4](API.md)). و«في أي معهد يعمل هذا المستخدم» صارت
تُقرأ من موضعٍ واحد يقرؤه السطحان (`User::instituteIds()`) بدل نسختين في `PanelScope` و`ApiScope`.

⬜ **وأثرٌ ثانٍ أثقل ما زال قائماً: سطحُ الإدارة كلُّه بلا نقطة API** — المعاهدُ والمستخدمون
والأدوارُ وبياناتُ الدخول تُكتب من اللوحة عبر `app/Actions/` مباشرةً ([CLIENTS.md §2](CLIENTS.md)).
فالجدولُ أعلاه يصف **سطحين**، وقرارُ 2026-09-07 يوجب أن يصير للسطح الثاني ما للأول من أبواب — وهو
نطاق **م.6.2**.
...
| **بلا موجّه مسارات في العملاء — الديسكتوب أيضاً** 🔄 م.6.3 | أُعيدت المسألةُ عند أعقد سطحٍ عميل ثم حُسمت كما حُسمت في م.5.3: نافذةٌ واحدة على ويندوز بلا روابطَ عميقة ولا شريطِ عنوان لا تستفيد ممّا يبيعه `go_router`، و`IndexedStack` فوق `Navigator` لكل باب يعطي مكدّساً مستقلاً لكل قسم — وهو نفسُ ما يلفّه `StatefulShellRoute`. والأبوابُ تُرشَّح بـ`user.permissions` في زمن التشغيل، فجدولُ مساراتٍ ثابت كان سيكتب الحارسَ مرّتين ([CHECKPOINT-PHASE-6.3.MD §2](CHECKPOINT-PHASE-6.3.MD)) |
| **الفرقُ بين الأستاذ والديسكتوب معاملٌ في مستودعٍ واحد** 🔄 م.6.4 | امتحانُ قرار «يُرفع ولا يُنسخ» على ـ٩٢٨ سطراً: `TeacherRepository` صار `CircleRepository` في `mousqe_core`، والفرقُ معاملات — `teacherUuid` فارغاً يعني المعهد كلّه، و`editableBy(canAmend:)` يقرأ الصلاحيةَ من اللقطة بدل أن تُفترض في الشيفرة. **والشاشاتُ لا تُرفع**: بطاقاتُ إبهامٍ على هاتف ليست جدولاً على مكتب — منطقٌ يُرفع لا تخطيط ([CHECKPOINT-PHASE-6.4.MD §2](CHECKPOINT-PHASE-6.4.MD)) |
| **قناةُ الكتابة تحدّ المستودعَ لا الموضوع** 🔄 م.6.5 · **وستّةٌ بعد م.6.6** | صار للديسكتوب أربعةُ مستودعات، وحدُّها ليس «الطلاب» و«الحلقات» بل **أوف-لاين أم متّصل**: `CircleRepository` و`StudentRepository` يقرآن drift ويكتبان في الطابور، و`CatalogRepository` يقرأ drift ويكتب REST، و`AdminRepository` يقرأ الشبكةَ ويكتب REST. وهو تجسيدُ قاعدة [API.md §3.10.1](API.md) في الشيفرة — فمن يضيف كتابةً يسأل أوّلاً «أتقبل التأخير؟» لا «أيّ شاشةٍ هذه؟». وثمنُ الكتابةِ المتّصلة أن أثرَها يعود في `sync/pull` لا في ردّ الطلب، فيُدفع في `runConnected`: مزامنةٌ فورية بعد النجاح، وتمييزُ «لا شبكة» من «الخادم رفض» ([CLIENTS.md §5](CLIENTS.md)) |
| **`mousqe_ui` لا تعتمد `mousqe_core`** 🔄 م.6.3 | حزمةُ الهوية البصرية تستقبل حالةً جاهزة ولا تعرف طابورَ المزامنة ولا نماذجَ الـ API؛ فتُستعمل في شاشةٍ بلا مزامنة وتُختبَر بلا قاعدة بيانات. وثمنُه ٤٥ سطرَ ربطٍ في كل تطبيق — وهو حقنُ اعتمادياتٍ لا منطق. وما يحتاجه الاثنان من **منطق** يُرفع إلى `mousqe_core` ولا يُنسخ بين التطبيقات |
| **الإحصاءُ يُحسب على الجهاز لا يُطلَب من الخادم** 🔄 م.6.6 | `circle_daily_stats` و`circle_cumulative_stats` **لا يُزامَنان** بقرارٍ قديم («نقلُ ما يمكن حسابُه نقلٌ زائد»)، وم.6.6 أوّلُ من دفع ثمنَه: فبُني `StatsRepository` نظيراً لهما من `attendances` — وهو المستودعُ الخامس، وقناتُه **يقرأ drift ولا يكتب شيئاً**. والبديلُ — نقطةُ إحصاءٍ في الـAPI — كان يجعل الداشبوردَ البابَ الوحيد الذي لا يُفتح بلا شبكة. **وما يشتريه ذلك المعادلةُ الواحدة**: `AttendanceRate` نقلٌ حرفيٌّ لـ`App\Support\AttendanceRate`، فرقمُ الداشبورد ورقمُ الورقة المطبوعة ورقمُ اللوحة حسابٌ واحد ([CHECKPOINT-PHASE-6.6.MD §2](CHECKPOINT-PHASE-6.6.MD)) |
| **ما لا يُزامَن ويلزم الحسابَ المحلي يُحمَل في `/bootstrap`** 🔄 م.6.6 | ثلاثةُ حقولٍ حتى اليوم — `theme` و`attendance.late_grace_minutes` (م.5.1) و**`points`** (م.6.6) — وكلُّها من `institutes.settings`، وهو جدولٌ لا يعبر `change_log`. والقاعدةُ استُخلصت بعد تكرارها ثلاثاً: **ما يُحسب محلياً يحتاج ثابتاً خادمياً**، فبدونها يحسب الجهازُ بافتراضيّاته ويخالف رقمُه رقمَ اللوحة **بلا خطأ يُرمى** ([SYNC-PROTOCOL.md §7](SYNC-PROTOCOL.md)) |
