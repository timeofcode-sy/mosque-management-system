...
الحالة: **محدَّث حتى المرحلة 5.4** — كل الجداول والنماذج والبذور والاختبارات موجودة في `backend/`.
...
و`sync_conflicts` يحفظ القيمة المُستبدَلة لعرضها على المشرف — وفيه `institute_id` 🔄 م.2026-09-07
ليرى كلُّ معهدٍ تعارضاتِه وحدها، و`resolution` تصير `client_wins` إن قلب المشرفُ الحكم
([SYNC-PROTOCOL.md §5](SYNC-PROTOCOL.md)).

🔴 **م.5.3 — وهو تيّارُ تغييرات لا لقطةُ حالة.** `SyncPull` يقرأ هذا الجدول **وحده** ولا يمسّ
جداول الدومين، فـ`since=0` يعني «التيّارَ من أوّله» لا «كلَّ ما في القاعدة». وعليه فالصفُّ الذي
لم يمرّ بمراقب `RecordsSyncChanges` — بياناتُ ما قبل م.5.1، أو بذرةٌ عُطّل فيها التسجيل، أو إدراجٌ
بالجملة لا يُطلق أحداث Eloquent — **لا سبيل لأي عميل أن يعرفه أبداً**. يردمه
`php artisan sync:backfill-change-log` — [SYNC-PROTOCOL.md §10](SYNC-PROTOCOL.md) البند 9.

🔄 **م.5.1: التسجيل على مستوى الصفّ لا الجلسة**، وأثرٌ بنيويٌّ على كل نموذج ينفّذ
`App\Contracts\Syncable` لا استدعاءٌ يدوي في `SyncPush` — فكتاباتُ اللوحة تصل العملاء أيضاً.
