...
([CLIENTS.md §2](CLIENTS.md)). صار لها **أربعٌ وعشرون نقطة REST** (§3.10 — تسعَ عشرةَ في م.6.2
وخمسٌ للمناهج في م.6.5) و**أربعةُ أنواع عمليات** في `sync/push` (§6)، والقسمةُ بينهما قاعدةٌ مُعلَنة
لا اجتهادُ كلِّ شاشة (§3.10.1).
...
| `GET` | `/guardian/children/{student_uuid}/attendance` | 🔄 م.7.1 — ملخّصُ ابنٍ ومنحناه وآخر 30 سجلاً | `role:guardian` |
| `GET` | `/guardian/children/{student_uuid}/progress` ✅ م.7.1 | محفوظاتُ ابنٍ مجمّعةً بالمنهج | `role:guardian` |
| `GET` | `/guardian/excuses` ✅ م.7.1 | أعذارُ أبنائه وحالتُها وجوابُ الطاقم | `role:guardian` |
...
| `GET` | `/student/me/standing` ✅ م.8.1 | رتبتُه في حلقته — **رقمٌ لا كشف** | `role:student` |
| `GET` | `/student/me/points` ✅ م.8.1 | نقاطُه بمصادرها في الدورة الجارية | `role:student` |
| `GET` | `/student/me/announcements` ✅ م.8.1 | إعلاناتُ معهده وحلقته | `role:student` |
...
| **المناهج ✅ م.6.5** — §3.10.5 | | | |
| `POST` | `/admin/curricula` | إنشاءُ منهج | `permission:curricula.manage` |
| `PUT` | `/admin/curricula/{uuid}` | تحريرُ منهج — والعامُّ يُرفض 422 | `permission:curricula.manage` |
| `POST` | `/admin/curricula/{uuid}/items` | بندٌ جديد — والقرآنُ يُرفض 422 | `permission:curricula.manage` |
| `PUT` | `/admin/curriculum-items/{uuid}` | تحريرُ بند — واسمُ الجزء مسموح | `permission:curricula.manage` |
| `DELETE` | `/admin/curriculum-items/{uuid}` | حذفُ بند — والمرتبطُ بمحفوظاتٍ يُرفض 422 | `permission:curricula.manage` |
...
// 200
{
  ...
  "institute": {
    ...
    "points": {                                 // ✅ م.6.6 — ثوابتُ حساب النقاط
...
**`points` ثوابتُ حسابٍ لا إعداداتُ عرض** ✅ م.6.6 — وهي **ثالثةُ مرّةٍ يتكرّر فيها النمط نفسُه**
في هذا الحقل: `theme` (م.5.1) و`attendance.late_grace_minutes` (م.5.1) وهذه. والقاعدةُ الجامعة
بينها واحدة: **ما يحسبه العميلُ محلياً يحتاج ثابتاً خادمياً، و`institutes.settings` جدولٌ لا
يُزامَن** ([SYNC-PROTOCOL.md §2](SYNC-PROTOCOL.md)) — فلا يبلغه إلا اللقطة.

وما يحتاجها هو **تقريرُ النقاط المطبوع من الديسكتوب**: عمودُ الحضور فيه مشتقٌّ يحسبه الجهاز من
صفوف `attendances` ([SYNC-PROTOCOL.md §7](SYNC-PROTOCOL.md))، فلولا هذه القيم لَحسبه بافتراضيّات
الحزمة — فيخالف رقمُ الورقة المطبوعة من الجهاز رقمَ اللوحة في **كل معهدٍ عدّل نقاطَه**، بلا خطأ
يُرمى ولا سطرَ سجلّ. وعميلٌ يتحدّث خادماً أقدم يبقى عاملاً بـ`PointsSettings::DEFAULTS` نفسِها:
الحقلُ الغائب لا يُسقط اللقطة.

**والأرقامُ الصحيحة تصل بلا كسر**: `json_encode(7.0) === '7'`، فيقرؤها العميل `num` لا `double`.
...
> 🔑 **م.7.1: هذه النقاطُ الأربعُ هي كلُّ مصادر تطبيق ولي الأمر — لا يستدعي `sync/pull` إطلاقاً.**
>
> التيّارُ يبثّ **معهداً كاملاً** لكل الأدوار ([SYNC-PROTOCOL.md §4](SYNC-PROTOCOL.md))، وهو على
> جهاز الأستاذ حِملٌ وعلى جهاز الأب **تسريب**: أسماءُ كلِّ طلاب المعهد وحضورُهم في هاتف كلِّ
> وليّ، وهو ليس طاقماً. ولا يحتاجه أصلاً — من يستدعي التيّارَ من **يكتب** في الطابور ليعرف ما
> حُسم من كتابته، ووليُّ الأمر لا يملك `sync.push`.
>
> ولذلك تعيد هذه النقاطُ **ما يُعرَض جاهزاً** — الملخّصَ والمنحنى محسوبين — لا صفوفاً يجمعها
> العميل. والأوف-لاين يبقى قائماً: العميلُ يخزّن استجابةَ كلِّ نقطةٍ لقطةً في drift ويقرأ منها
> بلا شبكة. القرارُ وحجّتُه في [PHASE-7-STAGES.MD §0.1](PHASE-7-STAGES.MD).
...
`GET /guardian/children/{student_uuid}/attendance` — 🔄 **م.7.1: صار شكلُه شكلَ `/student/me/attendance`
(§3.7) حرفياً**، وكان `{"data":[ … ]}`. الشاشتان تعرضان الشيءَ نفسَه لجمهورين — الأبُ يرى نسبةَ ابنه
والابنُ يرى نسبتَه — وعقدان مختلفان لنفس الرقم يعنيان حسابَه مرّتين. ابنٌ ليس له ⇒ 404:

```jsonc
{
  "summary": { "present": 84, "absent": 6, "late": 3, "excused": 2,
               "total": 95, "rate": 93.5 },
  "trend":  [ { "date": "2026-08-08", "rate": 100.0, "sessions": 1 },
              { "date": "2026-08-09", "rate": null,  "sessions": 0 }, … ],   // 30 يوماً
  "recent": [ …AttendanceResource… ]                                        // آخر 30 سجلاً
}
```

**و`session_date` صار في `AttendanceResource`** 🔄 م.7.1 — ولم يكن. المورد كان يعيد `recorded_at`
وحده، وهو **لحظةُ كتابة الأستاذ لا يومُ الحضور**: أستاذٌ يصحّح جلسةَ أمس اليوم يكتب `recorded_at`
اليومَ، فيقع السجلُّ في اليوم الخطأ عند من يقرأ يوماً بيوم. كان يكفي شاشةَ الأستاذ لأنها تفتح
يوماً بعينه فتعرفه مسبقاً، ولا يكفي وليَّ الأمر ولا الطالب. **والحقلُ `whenLoaded`**: يظهر حيث
حُمِّلت العلاقةُ ويغيب حيث لم تُحمَّل، فلا n+1 صامت في القوائم الطويلة.

`GET /guardian/children/{student_uuid}/progress` ✅ **م.7.1** ⇒ `{"data": { "<اسم المنهج>": [ ProgressResource ] }}`
— مقصورةً على `in_progress` و`memorized` و`mastered`، ونظيرتُها `/student/me/progress` (§3.7).

`GET /guardian/excuses` ✅ **م.7.1** ⇒ `{"data": [ AbsenceExcuseResource ]}` — الأحدثُ أوّلاً، ≤50:

```jsonc
{ "uuid": "0199…", "student_uuid": "0199…", "student_name": "محمد بن خالد",
  "from_date": "2026-09-08", "to_date": "2026-09-10", "reason": "سفر عائلي",
  "status": "rejected", "status_label": "مرفوض",          // pending | approved | rejected
  "reviewed_at": "2026-09-09T07:10:00.000000Z",
  "review_note": "الدورة في أيامها الأخيرة.",             // جوابُ الطاقم — بدونه رفضٌ بلا سبب
  "submitted_at": "2026-09-07T19:44:00.000000Z" }
```

**ولماذا نقطةٌ أصلاً؟** لأن حالةَ الإذن كانت ستصل في `sync/pull`، ووليُّ الأمر خرج من التيّار
كلِّه. بدونها يقدّم إذناً ثم **لا يرى جوابَه أبداً** — وهي ميزتُه الوحيدة التي يكتب فيها.
**والنطاقُ أبناؤه لا ما قدّمه هو**: المشرفُ يقدّم الإذنَ نيابةً عنه حين يصل هاتفياً، وذاك إذنٌ
عن ابنه يحقّ له أن يتابعه.
...
**والحصرُ في الاستعلام لا في العرض** في النقاط الأربع كلِّها: كلُّ قراءةٍ تبدأ من
`guardian->students()`، وطلبُ ابنٍ ليس له ⇒ **404 لا 403** — لأن 403 يؤكّد وجودَ الطالب (§4).
...
```jsonc
{
  ...
  "trend":  [ { "date": "2026-08-08", "rate": 100.0, "sessions": 1 },
              { "date": "2026-08-09", "rate": null,  "sessions": 0 }, … ],   // 30 يوماً بالضبط
  "recent": [ …AttendanceResource… ]                // آخر 20
}
```

🔑 **م.7.1: يومٌ بلا جلسة `rate: null` لا `0.0`** — وكان صفراً. هذه قاعدةُ م.6.6 حرفياً («ما لم
يُسجَّل لا يُفترض، و`null` ليست صفراً» — [CHECKPOINT-PHASE-6.6.MD §3](CHECKPOINT-PHASE-6.6.MD))
مطبَّقةً على السطح الخادمي بعد أن طُبِّقت في `StatsRepository` على سطح الديسكتوب. الصفرُ كان يقول
«حضر صفراً بالمئة» عن يومِ جمعةٍ لا تفقُّدَ فيه أصلاً؛ ولم يظهر أثرُه في اللوحة لأن مكوّنَ المنحنى
يرشّح دوائرَه بـ`sessions > 0` — لكنّ **الخطَّ الواصلَ** كان يهبط إليها، فيبدو منحنى طالبٍ مواظبٍ
مسنَّناً بين المئة والصفر. **ويومٌ كلُّ سجلّاته «مأذون» ⇒ `null` كذلك**: المأذونُ خارج المقام،
ويومٌ لا مقامَ له لا نسبةَ له. **فالعميلُ يميّز الحالتين**: `null` تُعرَض «لا جلسة» لا «٠٪».

`GET /student/me/progress` ⇒ `{"data": { "<اسم المنهج>": [ ProgressResource ] }}` — مجمَّعة باسم
المنهج العربي، ومقصورة على `in_progress` و`memorized` و`mastered`:

```jsonc
{ "uuid": "0199…", "item_name": "جزء عمّ", "item_code": "juz-30", "sort_order": 30,
  "status": "memorized", "status_label": "محفوظ", "percent": 100, "score": 92,
  "points": 10.0, "started_on": "2026-07-01", "completed_on": "2026-08-20",
  "achieved_on": null, "notes": null }
```

✅ **م.7.1: `ProgressResource` بُني، وكان مؤجَّلاً إلى «ما قبل م.8»** (§8). كانت هذه **النقطةَ
الوحيدة في النظام التي تعيد نموذجاً خاماً** — كلَّ أعمدة `student_curriculum_progress` بمفاتيحها
الداخلية (`student_id`، `curriculum_item_id`، `teacher_id`) وبشكلٍ غيرِ مثبَّتٍ بعقد. قُدِّم لأن
م.7.1 فتحت نقطةَ تقدُّمٍ ثانيةً لولي الأمر (§3.6)، ولا معنى لتثبيت شكلِ إحداهما وترك الأخرى — ولا
لمضاعفة غير المثبَّت. **ولا مفتاحَ داخلياً في الخرج**، و`points` رقمٌ لا نصّ (العمود `decimal:2`
يصل نصّاً «12.00» بلا تحويل).
...
#### 3.10.5 المناهجُ وبنودُها ✅ م.6.5

خمسُ نقاطٍ خلف `curricula.manage` — وهي **الفجوةُ المسمّاة** منذ م.6.2: `SaveCurriculumItem` قائمٌ
منذ م.4.5 ولا غلافَ له، فكان بابُ المناهج في الديسكتوب يحتاج متصفّحاً.

| النقطة | الجسم | الفعل |
|---|---|---|
| `POST · PUT /admin/curricula[/{uuid}]` | `{name, type, description?, sort_order?, is_active?}` | `SaveCurriculum` **جديد** |
| `POST /admin/curricula/{uuid}/items` | `{name, code?, sort_order?, count?}` | `SaveCurriculumItem` |
| `PUT · DELETE /admin/curriculum-items/{uuid}` | نفسُه · — | نفسُه · حذفٌ مباشر |

**والقناةُ REST لنفس حجج بنية الدورة (§3.10.1):** المنهجُ بنيةٌ يُبنى عليها — سجلُّ محفوظات الطالب
يُعلَّق على بنوده — ويُهيَّأ مرّةً في الفصل. ولا `GET` له: `curricula` و`curriculum_items` جدولان
يُزامَنان.

و`count` **مفتاحٌ واحدٌ في الحمولة ويُكتب في `meta` بمفتاح نوعه**: `hadiths` للحديث و`abyat`
للمتون، ولا عدّادَ لسواهما (م.4.5). و`meta` **تُدمج ولا تُستبدل**، فتحريرُ العدّاد لا يمحو
`meta.juz` الذي زرعته البذرة.

**وثلاثةُ أحكامٍ ترجع 422 برسالتها العربية:**

| الحكم | الرسالة |
|---|---|
| بندٌ جديدٌ أو محذوفٌ في منهج القرآن | «أجزاء القرآن ثابتة ولا تُضاف ولا تُحذف.» — وتحريرُ اسم جزءٍ قائمٍ **مسموح**: الثابتُ عددُها لا أسماؤها |
| بندٌ عليه سجلُّ محفوظات | «لا يمكن حذف بند مرتبط بسجل محفوظات.» |
| 🔴 تحريرُ **منهجٍ عامّ** من داخل معهد | «المناهج العامة لا تُحرَّر من داخل معهد…» — عطبٌ كشفته م.6.5، تفصيلُه في [CHECKPOINT-PHASE-6.5.MD §3](CHECKPOINT-PHASE-6.5.MD) |
...
| ~~**`/bootstrap` لا يخدم إلا الأستاذ**~~ (§3.4) | 🔄 **سُدّت نصفاً في م.6.1** (صارت تخدم من يملك `circles.view` — أي الديسكتوب)، و✅ **حُسم شقُّ ولي الأمر في م.7.1 قراراً لا كوداً: لا توسيع.** شاشتُه الأولى أبناؤه لا الحلقات، ويكفيه من اللقطة ما تعيده لكل الأدوار — الهويّةُ والمعهدُ وثيمُه؛ والأبناءُ لهم نقطتُهم (§3.6). ويبقى الطالبُ على `circles: []` حتى م.8 |
| ~~**`/student/me/progress` بلا Resource**~~ (§3.7) | ✅ **سُدّت في م.7.1** — `ProgressResource` بُني قبل موعده (كان «قبل م.8») لأن نقطةَ تقدُّمٍ ثانيةً فُتحت لولي الأمر، فلا يُضاعَف شكلٌ غيرُ مثبَّت |
| ⚠️ **رفعُ الشعار خارج الـ API** 🔄 م.6.2 · **بقي بعد م.6.5** (§3.10.2) | `logo_path` يُقرأ ولا يُكتب: الرفعُ عقدٌ آخر (`multipart/form-data`) لم يُفتح بعد. وقد بُنيت شاشةُ «بيانات المعهد» في م.6.5 **بلا بابِ شعار**، وفيها بطاقةٌ تقول إنه يُرفع من اللوحة — فصار **استثناءً موثَّقاً معروضاً لصاحبه** لا فجوةً صامتة. يُفتح إن طُلب رفعُ ملفٍّ آخر (صورةُ طالب مثلاً) |
| ~~**المناهجُ بلا نقطة**~~ | ✅ **سُدّت في م.6.5** — خمسُ نقاطٍ خلف `curricula.manage` (§3.10.5)، ومعها `SaveCurriculum` أُخرج من شاشة اللوحة |
...
| **الديسكتوب** (م.6) | نفسها + §3.8 و§3.9 و**§3.10 كاملاً بأقسامه الخمسة** (ومنها **§3.10.5 المناهج** ✅ م.6.5) · `attendance.teacher.take` و`amend` و**`attendance.session.lock`** ✅ م.6.4 في §6 · §4 كاملاً (الترويسة والنطاق) · `failed[]` وأنواعُ الإدارة الأربعة في §6 |
| **الأهل** (م.7) | **§3.6 كاملاً — وهو كلُّ مصادره** · §3.1 و§3.3 و§3.4 للدخول واللقطة · §5 رموزُ الأخطاء. 🔄 **ولا `sync/pull` ولا `push`** (م.7.1، §3.6) |
| **الطالب** (م.8) | §3.7 — و`ProgressResource` صار مثبَّتاً ✅ م.7.1 · `sync/pull` بلا `push`. ⚠️ **وتُعاد مسألةُ §3.6 عنده**: هل يخرج من التيّار كوليّ الأمر؟ حجّتُه أقوى (يقرأ عن نفسه وحدها) وما يعوزه أن ترتيبَه في الحلقة يُحسب محلياً من صفوف زملائه |
...
## 10. نقاطُ الطالب الثلاثُ الجديدة ✅ م.8.1

> 🔑 **الطالبُ يخرج من `sync/pull` كوليّ الأمر — وحجّتُه أقوى**: وليُّ الأمر يقرأ عن أبنائه وهم
> أكثرُ من واحد، والطالبُ يقرأ عن **نفسه وحدها**. فنقاطُ `/student/me/*` الخمسُ هي كلُّ مصادر
> تطبيقه، و drift **مخزنُ لقطاتها**
> ([CHECKPOINT-PHASE-8.1.MD §2](CHECKPOINT-PHASE-8.1.MD)).

`GET /student/me/standing` — **رقمٌ لا كشف**:

```jsonc
{ "data": { "circle_name": "حلقة أبي بن كعب",
            "rank": 5,        // null ⇒ لم يُقَس بعد — لا «الأخير»
            "peers": 20,
            "rate": 87.5,     // null مع rank
            "points": 42 } }
```

🔑 **والرتبةُ تُحسب في الخادم عمداً.** [APPS-FEATURES.md §6.2](APPS-FEATURES.md) كان يقول «يُحسب
محلياً» — وثمنُ ذلك أن يستقبل هاتفُ الطالب **صفوفَ حضورِ كلِّ زملائه** ليستخرج منها رقماً واحداً.
فقُلب الاتجاه: الخادمُ يحسب والجهازُ يعرض، **ولا يخرج من النقطة اسمُ زميلٍ ولا معرّفُه ولا نسبتُه**
— واختبارُ العقد يحرس ذلك بالنصّ.

**والمعادلةُ `AttendanceRate` نفسُها** التي تحسب نسبةَ اللوحة وتقريرِ الحلقة، **والتعادلُ يأخذ
الرتبةَ نفسَها** كما في تقرير النقاط منذ م.4.5.

`GET /student/me/points` ⇒ `{"data": {"quran": …, "hadith": …, "mutun": …, "attendance": …,
"manual": …, "total": …}}` — بمصادرها الخمسة في **الدورة الجارية**. ومعهدٌ بين دورتين ⇒ أصفارٌ لا
خطأ.

`GET /student/me/announcements` ⇒ `{"data": [ AnnouncementResource ]}` — الأحدثُ أوّلاً، ≤30:

```jsonc
{ "uuid": "0199…", "title": "مسابقة حفظ جزء عمّ", "body": "…",
  "scope": "circle",                       // all | circle
  "published_at": "2026-09-09T10:00:00.000000Z" }
```

**والترشيحُ في الخادم لا في العميل**: إعلانٌ وُجِّه إلى حلقةٍ ليست له **لا يصل جهازَه أصلاً**.
**ولا `scope_ids` في الخرج** — معرّفاتُ الحلقات الأخرى المشمولة ليست من شأن قارئه.
**والمسودّةُ ليست إعلاناً**: `published_at` فارغةٌ أو في المستقبل لا تصل قارئاً.

> 🔴 **وجدولُ `announcements` كان قائماً منذ م.1 بلا طرفَين**: لا شاشةَ تكتب ولا نقطةَ تقرأ —
> حمولةٌ ميتة نظيرُ `report_templates` المحذوف في م.4.5. فبُني طرفاه معاً في م.8.1: شاشةٌ في
> اللوحة خلف `announcements.manage`، وهذه النقطة.
