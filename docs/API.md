...
الحالة: **2026-09-06 · محدَّث حتى المرحلة 4.7** — ✅ 14 نقطة منفَّذة ومغطّاة باختبارات
...
// 200
{
  "institute": { "uuid": "0199…", "name": "معهد النور", "logo_path": null },
...
| `attendance.take` | `session_uuid` · `attendances[]{student_uuid, status, late_minutes?, note?, recorded_at?}` · `amend?` | `recorded_at` **هو مفتاح حسم التعارض** — أرسله دائماً بزمن الجهاز وقت التسجيل، لا وقت الدفع |
...
|---|---|
| **`user.roles` تعود `[]` دائماً** في `/auth/login` و`/auth/me` | المساران خارج `institute.scope`، فمفتاح فريق spatie غير مضبوط، و`roles()` في spatie 8 تُصفّي بـ`wherePivot(institute_id, …)` ⇒ لا صفّ. (اختبار `Api/AuthTest` يمرّ لأنه ضبط المفتاح في عملية الاختبار قبل الطلب.) **لا تبنِ توجيه التطبيق على هذا الحقل** — استعمل نجاح `/teacher/circles` مقابل 404، أو أضف الدور إلى `/bootstrap` قبل المرحلة 5 |
...
| **`/bootstrap` لا يخدم إلا الأستاذ** (§3.4) | م.7 وم.8 يحتاجان توسيعها أو نقاطاً بديلة |
